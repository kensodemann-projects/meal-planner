<template>
  <v-alert v-if="!today" type="error" variant="tonal" class="mb-4">
    Invalid or missing date parameter. Please provide a valid date in the URL (format: YYYY-MM-DD).
  </v-alert>
  <template v-else>
...
import { intlFormat, parseISO, isValid } from 'date-fns';
import { ref } from 'vue';
...
const today = dt && isValid(parseISO(dt)) ? parseISO(dt) : null;
