import { mount, VueWrapper } from '@vue/test-utils';
import { afterEach, describe, expect, it } from 'vitest';
import { createVuetify } from 'vuetify';
import * as components from 'vuetify/components';
import * as directives from 'vuetify/directives';
import SelectAutocomplete from '../SelectAutocomplete.vue';

const vuetify = createVuetify({ components, directives });

const defaultProps = {
  label: 'Food',
  items: ['Apple', 'Banana', 'Apricot', 'Cherry'],
  rules: [],
};

const createWrapper = (props: Record<string, unknown> = {}, modelValue: unknown = null) => {
  return mount(SelectAutocomplete, {
    props: { ...defaultProps, ...props, modelValue },
    global: { plugins: [vuetify] },
  });
};

describe('SelectAutocomplete', () => {
  let wrapper: VueWrapper<any>;

  afterEach(() => wrapper?.unmount());

  it('renders a VAutocomplete with the given label', () => {
    wrapper = createWrapper({ label: 'Ingredient' });
    const autocomplete = wrapper.findComponent({ name: 'VAutocomplete' });
    expect(autocomplete.props('label')).toBe('Ingredient');
  });

  it('passes items to VAutocomplete', () => {
    wrapper = createWrapper();
    const autocomplete = wrapper.findComponent({ name: 'VAutocomplete' });
    expect(autocomplete.props('items')).toEqual(defaultProps.items);
  });

  it('passes rules to VAutocomplete', () => {
    const rules = [(v: string) => !!v || 'Required'];
    wrapper = createWrapper({ rules });
    const autocomplete = wrapper.findComponent({ name: 'VAutocomplete' });
    expect(autocomplete.props('rules')).toEqual(rules);
  });

  it('passes item-title and item-value to VAutocomplete', () => {
    const items = [
      { id: 1, label: 'Apple' },
      { id: 2, label: 'Banana' },
    ];
    wrapper = createWrapper({ items, itemTitle: 'label', itemValue: 'id' });
    const autocomplete = wrapper.findComponent({ name: 'VAutocomplete' });
    expect(autocomplete.props('itemTitle')).toBe('label');
    expect(autocomplete.props('itemValue')).toBe('id');
  });

  describe('Tab key — selectFirstItem', () => {
    it('selects the first matching item on Tab when search matches', async () => {
      wrapper = createWrapper();
      const input = wrapper.find('input[role="combobox"]');

      await input.setValue('ap');
      await input.trigger('keydown.tab');

      expect(wrapper.emitted('update:modelValue')?.[0]).toEqual(['Apple']);
    });

    it('matching is case-insensitive', async () => {
      wrapper = createWrapper();
      const input = wrapper.find('input[role="combobox"]');

      await input.setValue('BAN');
      await input.trigger('keydown.tab');

      expect(wrapper.emitted('update:modelValue')?.[0]).toEqual(['Banana']);
    });

    it('keeps the current value on Tab when search is empty', async () => {
      wrapper = createWrapper({}, 'Apple');
      const input = wrapper.find('input[role="combobox"]');

      await input.setValue('');
      await input.trigger('keydown.tab');

      // Vuetify keeps search in sync with the selected title, so Tab re-selects Apple
      expect(wrapper.emitted('update:modelValue')).toBeUndefined();
    });

    it('sets modelValue to null on Tab when search matches nothing', async () => {
      wrapper = createWrapper({}, 'Apple');
      const input = wrapper.find('input[role="combobox"]');

      await input.setValue('zzz');
      await input.trigger('keydown.tab');

      const emissions = wrapper.emitted('update:modelValue');
      expect(emissions?.[emissions.length - 1]).toEqual([null]);
    });

    describe('object items', () => {
      const items = [
        { id: 1, label: 'Apple' },
        { id: 2, label: 'Banana' },
        { id: 3, label: 'Apricot' },
      ];

      it('selects the item-value of the first title match', async () => {
        wrapper = createWrapper({ items, itemTitle: 'label', itemValue: 'id' });
        const input = wrapper.find('input[role="combobox"]');

        await input.setValue('ap');
        await input.trigger('keydown.tab');

        expect(wrapper.emitted('update:modelValue')?.[0]).toEqual([1]);
        expect((input.element as HTMLInputElement).value).toBe('Apple');
      });

      it('matching against item-title is case-insensitive', async () => {
        wrapper = createWrapper({ items, itemTitle: 'label', itemValue: 'id' });
        const input = wrapper.find('input[role="combobox"]');

        await input.setValue('BAN');
        await input.trigger('keydown.tab');

        expect(wrapper.emitted('update:modelValue')?.[0]).toEqual([2]);
      });

      it('sets modelValue to null on Tab when search matches nothing', async () => {
        wrapper = createWrapper({ items, itemTitle: 'label', itemValue: 'id' }, 1);
        const input = wrapper.find('input[role="combobox"]');

        await input.setValue('zzz');
        await input.trigger('keydown.tab');

        const emissions = wrapper.emitted('update:modelValue');
        expect(emissions?.[emissions.length - 1]).toEqual([null]);
      });

      it('uses the title as the value when only item-title is specified', async () => {
        wrapper = createWrapper({ items, itemTitle: 'label' });
        const input = wrapper.find('input[role="combobox"]');

        await input.setValue('ap');
        await input.trigger('keydown.tab');

        expect(wrapper.emitted('update:modelValue')?.[0]).toEqual(['Apple']);
      });

      it('uses the default title property when only item-value is specified', async () => {
        const titledItems = [
          { title: 'Apple', id: 1 },
          { title: 'Banana', id: 2 },
          { title: 'Apricot', id: 3 },
        ];
        wrapper = createWrapper({ items: titledItems, itemValue: 'id' });
        const input = wrapper.find('input[role="combobox"]');

        await input.setValue('ap');
        await input.trigger('keydown.tab');

        expect(wrapper.emitted('update:modelValue')?.[0]).toEqual([1]);
      });
    });
  });
});
